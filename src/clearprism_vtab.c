    ...
    if (vtab->load_extension) {
        ...
        if (ext_rc != SQLITE_OK) {
            sqlite3_log(SQLITE_WARNING,
                "clearprism: extension load failed during schema discovery: %s",
                ext_err ? ext_err : "unknown");
            sqlite3_free(ext_err);
            /* Non-fatal: continue without extension — schema discovery may still
...
static int vtab_init_subsystems(clearprism_vtab *vtab, char **pzErr)
{
    ...
    if (vtab->cache_db_path) {
        ...
        if (!l2) {
            ...
            vtab->l2_active = 1;
            /* Start background refresh thread — initial populate runs async,
             * first query serves from sources directly */
            char *start_err = NULL;
